...
from gnn_egnn.gcl import GCL, EGCL #, EGCL_vel, GCL_rf_vel
...
class GNN(nn.Module):
    def __init__(self, input_dim, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, attention=0, recurrent=False):
        ...
        m = nn.AvgPool1d(node_size, stride=node_size)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.LeakyReLU(0.2), n_layers=4, coords_weight=1.0):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, EGCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, act_fn=act_fn, recurrent=True, coords_weight=coords_weight))
        self.to(self.device)
...
#         self.embedding = nn.Linear(in_node_nf, self.hidden_nf)
#         for i in range(0, n_layers):
#             self.add_module("gcl_%d" % i, EGCL_vel(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent, norm_diff=norm_diff, tanh=tanh))
#         self.to(self.device)
